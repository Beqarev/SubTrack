...
public class Subscription : IValidatableObject
{
    ...
    [NotMapped]
    public bool IsPaused => Status == SubscriptionStatus.Paused;
    ...
    public int? TrialDaysRemaining
    {
        get
        {
            if (!IsFreeTrial || TrialEndDate is null)
            {
                return null;
            }

            return Math.Max(0, (TrialEndDate.Value.Date - DateTime.Today).Days);
        }
    }

    [NotMapped]
    public bool IsTrialExpiringSoon =>
        IsFreeTrial &&
        TrialDaysRemaining is not null &&
        TrialDaysRemaining <= ReminderDaysBeforeRenewal;

    [NotMapped]
    public bool IsBillDueSoon =>
        IsActive &&
        NextBillingDate.Date >= DateTime.Today &&
        (NextBillingDate.Date - DateTime.Today).Days <= ReminderDaysBeforeRenewal;
